...
namespace RPGTable.MapEditor
{
    ...
    public sealed class MapEditorMapController : MonoBehaviour
    {
        ...
        {
            var placedElements = FindObjectsByType<PlacedMapElement>(FindObjectsSortMode.None);
            var exitPoints = FindObjectsByType<MapExitPoint>(FindObjectsSortMode.None);
            var spawnZones = FindObjectsByType<MapSpawnZone>(FindObjectsSortMode.None);
        ...
        {
            foreach (var exitPoint in FindObjectsByType<MapExitPoint>(FindObjectsSortMode.None))
            {
        ...
        {
            foreach (var spawnZone in FindObjectsByType<MapSpawnZone>(FindObjectsSortMode.None))
            {
    ...
    public sealed class CampaignEditorController : MonoBehaviour
    {
        ...
        private void ReloadMapList()
        {
            ...
            foreach (var path in UserMapStore.GetMapPaths())
            {
                var capturedPath = path;
                CreateListButton(UserMapStore.GetDisplayName(path), () => AddMapNode(capturedPath, Vector2.zero, null));
            }
        }
        ...
        private void CreateListButton(string label, UnityEngine.Events.UnityAction action)
        {
            var buttonObject = new GameObject(label, typeof(RectTransform));
            buttonObject.transform.SetParent(mapListRoot, false);
    ...
    public sealed class CampaignBoardPanZoom : MonoBehaviour, IDragHandler, IScrollHandler
    {
        ...
        public void OnScroll(PointerEventData eventData)
        {
            ...
            var current = content.localScale.x;
            var next = Mathf.Clamp(current + eventData.scrollDelta.y * zoomSpeed, minScale, maxScale);
    ...
    public sealed class CampaignMapNode : MonoBehaviour, IDragHandler
    {
        ...
        private Vector2 PinPosition(Vector3 mapPosition)
        {
            ...
            {
                return mapPosition.x >= 0f
                    ? new Vector2(half.x, 0f)
                    : new Vector2(-half.x, 0f);
            }

            return mapPosition.y >= 0f
