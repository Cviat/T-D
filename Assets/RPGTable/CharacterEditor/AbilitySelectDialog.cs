...
{
    public sealed class AbilitySelectDialog : MonoBehaviour
    {
        public static void Show(Sprite dialogFrame, Sprite dialogBg, Action<string> onSelect)
        {
            var canvasObject = new GameObject("Ability Select Dialog", typeof(RectTransform));
            ...
            // Frame layer: Draw the gold frame
            var panelImg = panel.AddComponent<Image>();
            panelImg.color = Color.white;
            panelImg.sprite = dialogFrame != null ? dialogFrame : FindFrameSprite();

            // Background layer: Draw the dark textured background
            ...
            // Scroll View filling the background container
            ...
            // Tight grid layout (4 columns of 80x80 cells, centered)
            ...
            // Load abilities
            var abilityCards = Resources.LoadAll<AbilityCard>("AbilityCards");
            var matchingAbilities = new List<AbilityCard>();

            foreach (var card in abilityCards)
            {
                if (card != null && !string.IsNullOrEmpty(card.title))
                {
                    matchingAbilities.Add(card);
                }
            }

            // Cell index 0: Clear / Empty cell
            CreateAbilityCardButton(content.transform, "Снять умение", null, slotFrame, () =>
            {
                Destroy(canvasObject);
                onSelect?.Invoke(string.Empty);
            });

            int loadedCount = matchingAbilities.Count;
            int totalCells = Mathf.Max(16, loadedCount + 1); // Pad to minimum of 16 cells (4x4)

            // Populate matching abilities
            for (var i = 0; i < loadedCount; i++)
            {
                var card = matchingAbilities[i];
                var name = card.title;
                var icon = card.icon;
                CreateAbilityCardButton(content.transform, name, icon, slotFrame, () =>
                {
                    Destroy(canvasObject);
            ...
            {
                CreateAbilityCardButton(content.transform, string.Empty, null, slotFrame, null);
            }
        }
        ...
        private static void CreateAbilityCardButton(Transform parent, string label, Sprite itemIcon, Sprite slotFrame, Action action)
        {
            var cardObj = new GameObject(string.IsNullOrEmpty(label) ? "EmptySlot" : label, typeof(RectTransform));
            cardObj.transform.SetParent(parent, false);

            var cardRt = cardObj.GetComponent<RectTransform>();
            cardRt.sizeDelta = new Vector2(80f, 80f);
            ...
            else if (action != null && label == "Снять умение")
            {
